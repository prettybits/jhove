...
public class TextHandler extends HandlerBase {
    ...
    public void show(App app) {
        ...
        while (iter.hasNext()) {
            ...
            _writer.println(margin + " OutputHandler: " + handler.getName()
                    + " " + handler.getRelease());
        }
    ...
    public void show(RepInfo info) {
        ...
        _writer.println(margin + "RepresentationInformation: " + info.getUri());
        if (module != null) {
            this.reportingModule = module.getName();
            _writer.println(margin + " ReportingModule: " + this.reportingModule
        ...
        if (date != null) {
            _writer.println(margin + " LastModified: " + dateTime.format(date));
        }
        ...
        if (n > 0) {
            ...
            for (int i = 0; i < n; i++) {
                _writer.println(margin + "  " + list1.get(i));
            }
        }
        ...
        n = list3.size();
        if (n > 0) {
            _writer.print(margin + " Profile: " + list3.get(0));
            for (int i = 1; i < n; i++) {
                _writer.print(", " + list3.get(i));
            }
            _writer.println();
        }
        ...
        n = list4.size();
        for (int i = 0; i < n; i++) {
            showChecksum((Checksum) list4.get(i));
        }
     ...
     * multiple files between the header and the footer.
     */
    @Override
    ...
    public void showHeader() {
        ...
        _writer.println(margin + _app.getName() + " (Rel. " + _app.getRelease()
    ...
    private void showMessage(Message message) {
        ...
        if (!(id == null || id.isEmpty() || id.equals(JhoveMessages.NO_ID))) {
            ...
            _writer.println(margin + " InfoLink: " + Handlers.makeInfoLink(this.reportingModule, id));
        }
    ...
    private void showScalarProperty(Property property, String margin) {
        ...
        if (PropertyType.PROPERTY.equals(type)) {
            ...
            showAESAudioMetadata((AESAudioMetadata) property.getValue(), margin
                    + " ", _je.getShowRawFlag());
        } else if (PropertyType.TEXTMDMETADATA.equals(type)) {
            showTextMDMetadata((TextMDMetadata) property.getValue(), margin
                    + " ", _je.getShowRawFlag());
        } else {
        ...
        if (n > 0) {
            ...
            for (i = 0; i < n; i++) {
                if (valueIsProperty) {
                    ...
                    showTextMDMetadata((TextMDMetadata) list.get(i), margin
                            + " ", _je.getShowRawFlag());
                } else {
    ...
    private void showMapProperty(Property property, String margin) {
        /*
         * Map output looks like key : mapkey1 / mapval1, mapkey2 / mapval2, ...
         */
        PropertyType type = property.getType();
        boolean valueIsProperty = PropertyType.PROPERTY.equals(type);
        boolean valueIsNiso = PropertyType.NISOIMAGEMETADATA.equals(type);
        ...
        while (propiter.hasNext()) {
            ...
            if (valueIsProperty) {
                ...
                showNisoImageMetadata((NisoImageMetadata) val, margin + " ",
                        _je.getShowRawFlag());
            } else if (valueIsTextMD) {
                showTextMDMetadata((TextMDMetadata) val, margin + " ",
                        _je.getShowRawFlag());
            } else {
    ...
    private void showArrayProperty(Property property, String margin) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (PropertyType.BOOLEAN.equals(propType)) {
                ...
                showNisoImageMetadata(niso, margin + " ", _je.getShowRawFlag());
                continue;
            } else if (PropertyType.TEXTMDMETADATA.equals(propType)) {
                if (i == 0) {
                    _writer.println();
                }
                showTextMDMetadata(textMDArray[i], margin + " ",
                        _je.getShowRawFlag());
                continue;
            } else if (PropertyType.PROPERTY.equals(propType)) {
                if (i == 0) {
    ...
    /*
     * Output the AES audio metadata, which is its own special kind of property.
     */
    private void showAESAudioMetadata(AESAudioMetadata aes, String margin,
        ...
        if (lin != AESAudioMetadata.NULL) {
            _writer.println(margn2 + "FirstSampleOffset: " + Long.toString(lin));
        }
        ...
        if (!flist.isEmpty()) {
            ...
            if (bitDepth != AESAudioMetadata.NULL
                ...
                if (bitDepth != AESAudioMetadata.NULL) {
                    _writer.println(margn4 + "BitDepth: "
                            + Integer.toString(bitDepth));
                }
                if (sampleRate != AESAudioMetadata.NILL) {
                    _writer.println(margn4 + "SampleRate: "
                            + Double.toString(sampleRate));
                }
                if (wordSize != AESAudioMetadata.NULL) {
                    _writer.println(margn4 + "WordSize: "
                            + Integer.toString(wordSize));
                }
                if (bitRed != null) {
                    _writer.println(margn4 + "BitrateReduction");
                    _writer.println(margn5 + "CodecName: " + bitRed[0]);
                    _writer.println(margn5 + "codecNameVersion: " + bitRed[1]);
                    _writer.println(margn5 + "codecCreatorApplication: "
                            + bitRed[2]);
                    _writer.println(margn5 + "codecCreatorApplicationVersion: "
                            + bitRed[3]);
                    _writer.println(margn5 + "codecQuality: " + bitRed[4]);
                    _writer.println(margn5 + "dataRate: " + bitRed[5]);
                    _writer.println(margn5 + "dataRateMode: " + bitRed[6]);
                }
            }
        }
    }
    ...
    /*
     * Output the Niso image metadata, which is its own special kind of
     * property. This provides a text approximation to MIX 0.2.
     */
    private void showNisoImageMetadata02(NisoImageMetadata niso, String margin,
        ...
        if ((n = niso.getColorSpace()) != NisoImageMetadata.NULL) {
            _writer.println(margn2
                    ...
                    + addIntegerValue(n, NisoImageMetadata.COLORSPACE,
                            NisoImageMetadata.COLORSPACE_INDEX, rawOutput));
        }
        ...
        if ((n = niso.getPlanarConfiguration()) != NisoImageMetadata.NULL) {
            _writer.println(margn2
                    + "PlanarConfiguration: "
                    + addIntegerValue(n,
                            NisoImageMetadata.PLANAR_CONFIGURATION, rawOutput));
        }
        ...
        if ((n = niso.getFlash()) != NisoImageMetadata.NULL) {
            _writer.println(margn2 + "Flash: "
                    + addIntegerValue(n, NisoImageMetadata.FLASH, rawOutput));
        }
        ...
        if ((n = niso.getBackLight()) != NisoImageMetadata.NULL) {
            _writer.println(margn2
                    + "BackLight: "
                    + addIntegerValue(n, NisoImageMetadata.BACKLIGHT, rawOutput));
        }
        ...
        if ((n = niso.getAutoFocus()) != NisoImageMetadata.NULL) {
            _writer.println(margn2
                    + "AutoFocus: "
                    + addIntegerValue(n, NisoImageMetadata.AUTOFOCUS, rawOutput));
        }
        ...
        if ((n = niso.getSensor()) != NisoImageMetadata.NULL) {
            _writer.println(margn2 + "Sensor: "
                    + addIntegerValue(n, NisoImageMetadata.SENSOR, rawOutput));
        }
        ...
        if ((n = niso.getSourceXDimensionUnit()) != NisoImageMetadata.NULL) {
            _writer.println(margn2
                    ...
                    + addIntegerValue(n,
                            NisoImageMetadata.SOURCE_DIMENSION_UNIT, rawOutput));
        }
        ...
        if ((iarray = niso.getExtraSamples()) != null) {
            _writer.print(margn2
                    + "ExtraSamples: "
                    + addIntegerValue(iarray[0],
                            NisoImageMetadata.EXTRA_SAMPLES, rawOutput));
            for (int i = 1; i < iarray.length; i++) {
                _writer.print(", "
                        + addIntegerValue(iarray[i],
                                NisoImageMetadata.EXTRA_SAMPLES, rawOutput));
            }
            _writer.println();
        }
        ...
        if ((n = niso.getGrayResponseUnit()) != NisoImageMetadata.NULL) {
            _writer.println(margn2
                    + "GrayResponseUnit: "
                    + addIntegerValue(n,
                            NisoImageMetadata.GRAY_RESPONSE_UNIT_02, rawOutput));
        }
        ...
        if ((n = niso.getTargetType()) != NisoImageMetadata.NULL) {
            _writer.println(margn2
                    + "TargetType: "
                    + addIntegerValue(n, NisoImageMetadata.TARGET_TYPE,
                            rawOutput));
        }
    ...
    private void showNisoImageMetadata10(NisoImageMetadata niso, String margin,
        ...
        if (xres != NisoImageMetadata.NULL && yres != NisoImageMetadata.NULL) {
            ...
            _writer.println(margn2 + "MaximumOpticalResolution: "
                    + Double.toString(res));
        }
        if ((s = niso.getScanningSoftware()) != null) {
            _writer.println(margn2 + "ScanningSoftware: " + s);
    ...
    private String addIntegerValue(int value, String[] labels, int[] index,
            boolean rawOutput) {
        String s = null;
